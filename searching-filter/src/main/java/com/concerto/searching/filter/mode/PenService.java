...
public class PenService {
	...
	public ApiResponse<List<Pen>> searchAndFilter(Filter searchRequest) throws Exception {
		List<Pen> pen = generateMockData(searchRequest);

		return new ApiResponse<>(pen, pen.size());
	}

	private List<Pen> generateMockData(Filter searchRequest) throws Exception {
		List<Pen> mockItems = new ArrayList<>();

		mockItems.add(new Pen(1L, "ExamTime", "Cello"));
		mockItems.add(new Pen(2L, "Exam", "Lexy"));

		List<Pen> filteredItems = new ArrayList<>();

		Map<String, Method> fieldToGetterMap = new HashMap<>();
		fieldToGetterMap.put("name", Pen.class.getMethod("getName"));
		fieldToGetterMap.put("brand", Pen.class.getMethod("getBrand"));
		...
		for (Pen item : mockItems) {
			...
			for (Search filter : searchRequest.getFilters()) {
				...
				if (("name".equalsIgnoreCase(fieldName) && "brand".equalsIgnoreCase(fieldName))
						&& "eq".equalsIgnoreCase(operator)) {
					Method getter = fieldToGetterMap.get(fieldName.toLowerCase());

					if (getter != null) {
						String fieldValue = (String) getter.invoke(item);
				...
				// Add more conditions for other fields and operators here
			}
